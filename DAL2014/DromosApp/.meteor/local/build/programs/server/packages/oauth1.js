...
OAuth._requestHandlers['1'] = function (service, query, res) {                                         // 2
  ...
  if (query.requestTokenAndRedirect) {                                                                 // 12
    ...
    var callbackUrl = Meteor.absoluteUrl("_oauth/twitter?close&state=" +                               // 14
                                         query.state);                                                 // 15
                                                                                                       // 16
    // Get a request token to start auth process                                                       // 17
    oauthBinding.prepareRequestToken(callbackUrl);                                                     // 18
                                                                                                       // 19
    // Keep track of request token so we can verify it on the next step                                // 20
    OAuth._storeRequestToken(query.state,                                                              // 21
      oauthBinding.requestToken,                                                                       // 22
      oauthBinding.requestTokenSecret                                                                  // 23
    );                                                                                                 // 24
                                                                                                       // 25
    // support for scope/name parameters                                                               // 26
    var redirectUrl = undefined;                                                                       // 27
    if(typeof urls.authenticate === "function") {                                                      // 28
      redirectUrl = urls.authenticate(oauthBinding);                                                   // 29
    } else {                                                                                           // 30
      redirectUrl = urls.authenticate + '?oauth_token=' + oauthBinding.requestToken;                   // 31
    }                                                                                                  // 32
                                                                                                       // 33
    // redirect to provider login, which will redirect back to "step 2" below                          // 34
    res.writeHead(302, {'Location': redirectUrl});                                                     // 35
    res.end();                                                                                         // 36
  } else {                                                                                             // 37
    // step 2, redirected from provider login - store the result                                       // 38
    // and close the window to allow the login handler to proceed                                      // 39
                                                                                                       // 40
    // Get the user's request token so we can verify it and clear it                                   // 41
    var requestTokenInfo = OAuth._retrieveRequestToken(query.state);                                   // 42
                                                                                                       // 43
    // Verify user authorized access and the oauth_token matches                                       // 44
    // the requestToken from previous step                                                             // 45
    if (query.oauth_token && query.oauth_token === requestTokenInfo.requestToken) {                    // 46
                                                                                                       // 47
      // Prepare the login results before returning.  This way the                                     // 48
      // subsequent call to the `login` method will be immediate.                                      // 49
                                                                                                       // 50
      // Get the access token for signing requests                                                     // 51
      oauthBinding.prepareAccessToken(query, requestTokenInfo.requestTokenSecret);                     // 52
                                                                                                       // 53
      // Run service-specific handler.                                                                 // 54
      var oauthResult = service.handleOauthRequest(oauthBinding);                                      // 55
                                                                                                       // 56
      var credentialSecret = Random.secret();                                                          // 57
                                                                                                       // 58
      // Store the login result so it can be retrieved in another                                      // 59
      // browser tab by the result handler                                                             // 60
      OAuth._storePendingCredential(query.state, {                                                     // 61
        serviceName: service.serviceName,                                                              // 62
        serviceData: oauthResult.serviceData,                                                          // 63
        options: oauthResult.options                                                                   // 64
      }, credentialSecret);                                                                            // 65
    }                                                                                                  // 66
                                                                                                       // 67
    // Either close the window, redirect, or render nothing                                            // 68
    // if all else fails                                                                               // 69
    OAuth._renderOauthResults(res, query, credentialSecret);                                           // 70
  }                                                                                                    // 71
};                                                                                                     // 72
                                                                                                       // 73
/////////////////////////////////////////////////////////////////////////////////////////////////////////
